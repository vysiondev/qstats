...
func (b *BaseHandler) AutorespondToMap(s *discordgo.Session, m *discordgo.Message, mapID int, ctx context.Context) error {
	...
	e.AddTitle(fmt.Sprintf("[%s] %s - %s [%s]",
		...
		utils.RemoveFormattingCharacters(thisMap.Map.Artist),
		utils.RemoveFormattingCharacters(thisMap.Map.Title),
		utils.RemoveFormattingCharacters(thisMap.Map.DifficultyName),
	))
	e.AddTitleURL(bot_constants.QuaverMainSite + "/mapset/map/" + strconv.Itoa(int(thisMap.Map.ID)))
